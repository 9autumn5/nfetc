from NFETC2018.model_param_space import ModelParamSpace
from hyperopt import fmin, tpe, hp, STATUS_OK, Trials, space_eval
from optparse import OptionParser
from NFETC2018.utils import logging_utils, data_utils, embedding_utils, pkl_utils
from NFETC2018.utils.eval_utils import strict, loose_macro, loose_micro, label_path, complete_path
import numpy as np
...
from NFETC2018.nfetc import NFETC
...
class Task:
    def __init__(self, model_name, data_name, cv_runs, params_dict, logger):
        ...
        if data_name == "wiki":
            ...
            words_train, mentions_train, positions_train, labels_train = data_utils.load(config.TEST_TRAIN_CLEAN)
            words, mentions, positions, labels = data_utils.load(config.TEST_TEST_CLEAN)
            type2id, typeDict = pkl_utils._load(config.TEST_TYPE)
            num_types = len(type2id)
            type_info = config.TEST_TYPE
...
class TaskOptimizer:
    ...
    def run(self):
        trials = Trials()
        best = fmin(self._obj, self.model_param_space._build_space(), tpe.suggest, self.max_evals,timeout=None,loss_threshold=None,trials= trials)
