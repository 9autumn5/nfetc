...
from NFETC2018.utils import pkl_utils
import os
...
def preprocess(data_name, if_clean=False, full_path=False):
    if data_name == "wiki":
        ...
        raw_all_file = config.TEST_ALL
        raw_train_file = config.TEST_TRAIN
        raw_valid_file = config.TEST_VALID
        raw_test_file = config.TEST_TEST
        clean_train_file = config.TEST_TRAIN_CLEAN
        clean_test_file = config.TEST_TEST_CLEAN
        type_file = config.TEST_TYPE
